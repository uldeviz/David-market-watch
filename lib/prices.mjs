...
export async function fetchIntraday(symbolKey, rangeMinutes = 30) {
  ...
  // Timeout esplicito: senza questo, una risposta lenta o assente di Yahoo
  // blocca lo script a tempo indefinito (e' successo nel primo test reale).
  ...
  try {
    ...
    if (err?.name === "AbortError") {
      throw new Error(`Yahoo chart: timeout (10s) per ${symbolKey}`);
    }
    throw err;
  } finally {
