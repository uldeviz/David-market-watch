...
export function formatAlertText(event) {
  ...
  // Titolo tradotto se disponibile (MyMemory, salvato in ingest.mjs), ma il
  // rilevamento ticker/regione sopra resta SEMPRE sul testo originale
  // inglese — le parole chiave (REGION_KEYWORDS, DIRECTION_RULES, ecc.) sono
  // in inglese e non riconoscerebbero il testo tradotto.
  const displayTitle = event.title_it || event.title;

  return [header, displayTitle, `${time} · ${event.source}`, event.url || ""].filter(Boolean).join("\n");
}
...
export async function sendEmailAlert(event) {
  ...
  try {
    ...
    const { error } = await resend.emails.send({
      from,
      to,
      subject: `${emoji} [${event.impact_level}] ${event.title_it || event.title}`,
      text: formatAlertText(event),
    });
