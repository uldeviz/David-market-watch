...
export function formatAlertText(event) {
  ...
  // Uniti cosi' (invece che con un template a spazi fissi) per non lasciare
  // doppi spazi quando l'arrow e' assente — capitava e sembrava un errore.
  const headerParts = [emoji, assetLabel, arrow].filter(Boolean);
  const header = headerParts.length
    ? `${headerParts.join(" ")} — ${event.impact_level}`
